...
class PseudoBox_Head_lqe_ablation(AnchorFreeHead):
    def __init__(self,
                 num_classes,
                 in_channels,
    ...
    def _init_layers(self):
        ...
        #lpf add 20240416
#111        self.lqe_layer = nn.Conv2d(self.feat_channels, 1, 3, padding=1)
        #lpf add 20240620
    ...
    def init_weights(self):
        ...
        #lpf add 20240416
#111        normal_init(self.lqe_layer, std=0.01)
        #lpf add 20240620
        if self.use_feature_alignment:
...
#        cls_score = self.conv_cls(cls_feat)
 ...
 #111       quality_score = self.lqe_layer(reg_feat)
#111        cls_score = (cls_score.sigmoid()) * ((quality_score.sigmoid()).pow(0.75))  # lpf 0.8/0.2 1/0.3 0.2  1.0/0.75
        ...
        return cls_score, bbox_pred, centerness #111,quality_score
...
#111             quality_score,
        ...
        #lpf add 20240416
#111        flatten_quality_score = [
#111            quality_score_single.permute(0, 2, 3, 1).reshape(-1)
#111            for quality_score_single in quality_score
#111        ]
#111        flatten_quality_score=torch.cat(flatten_quality_score)
...
#111            loss_iou_weight=1.0
            # lqe_weight = lqe_target.new_ones(pos_inds.shape)
#111            lqe_loss_weight_switch=True   ## ablation 6: if weight lqe_loss and how to weight is : using which function
#111            if lqe_loss_weight_switch:
#111                loss_iou = loss_iou_weight*weighted_binary_cross_entropy(flatten_quality_score[pos_inds], lqe_target[pos_inds], weight=pos_centerness_targets_1,
#111                                                                     avg_factor=centerness_denorm)
#111            else:
#111                loss_iou = loss_iou_weight * weighted_binary_cross_entropy(flatten_quality_score[pos_inds],
#111                                                                           lqe_target[pos_inds],
#111                                                                           weight=torch.tensor(1),
#111                                                                           avg_factor=num_pos)
...
#111            loss_iou = pos_bbox_preds.sum()*0
        #lpf add 0905
